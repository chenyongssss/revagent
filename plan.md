...
| 2. Paper ingestion and Claim-Evidence Graph | Advanced partial | A versioned paper manifest binds sources/PDFs, records structural claim edges, maps SyncTeX pages, and supports hash-bound human semantic dependency reviews and experiment artifacts. Local PDF tools observe text-empty pages, font embedding, image pages, and boundary-overflow candidates. External DOI verification, complete high-risk traceability, and expert validation of semantic and rendering observations remain incomplete. |
...
2. Complete high-risk claim traceability and external DOI verification; the
   hash-bound semantic dependency, experiment evidence, and rendered PDF
   observation workflows are now implemented but still require expert review.
